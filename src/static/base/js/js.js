...
function initLike(){
    if($.cookie('like') === undefined){
       return;
        ...
        var sLike = $.cookie('like');
        var arrLike = sLike.split('|');
        for (var i = 0; i < arrLike.length; i ++){
            $(".fav > i[data-id|="+ arrLike[i] +"]").addClass('on');
        }
    }
}
//判断当前文章ID是否已被设置为喜欢
function checkLike(nId,sKey){
    var arrLike = sKey.split("|");
    if(arrLike.indexOf(nId) == -1){
        return true;
    } else {
        return false;
    }
...
$(function(){
    ...
    $('.J_bigPic > img').attr("src",sPic);
    $('.J_gallery > li').click(function(){
        var $this = $(this);
        var sPic = $this.children("img").attr("src");
        $('.J_bigPic > img').attr("src",sPic);
